...
# based on cadnano2pdb.pl by Jejoong Yoo 
...
usage="python cadnano2pdb.py --ff=(CHARMM|AMBER) --lattice=(honeycomb|square) [--scaf=SCAFFOLD_SEQUENCE] [--stap=STAPLE_CSV_FILE] [--help] JSON_FILE"
description='''
...
pwd=os.getcwd()
seq_files = glob.glob("%s/cadnano2pdb.seq.*.dat" % (pwd))

scafseq = {}
for f in seq_files:
    m = re.search(".*cadnano2pdb.seq.(.*).dat",f)
    name = m.group(1)
