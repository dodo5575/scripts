...
proc computeIon {selIon sectionMin sectionMax} {
    ...
    set sel [atomselect top "$selIon and y >= $sectionMin and y < $sectionMax"]
...
proc compute {name structPrefix outDir dcdFreq timestep startFrame xstPrefix dcdList} {
    ...
    set MinY [getMinY $xstPrefix]

    set Ymax [expr $MinY/2.0]
    set Ymin [expr -1*($MinY/2.0)]

    #divide into section
    if {[expr floor($Ymax)] == $Ymax} {
	set floorMax [expr $Ymax - 1]
    } else {
        set floorMax [expr floor($Ymax)]
    }
    set floorMin [expr floor($Ymin+1)]
	...
	foreach s $selText o $out {
            for {set secN 0} {$secN <= [expr $nSection - 1]} {incr secN} {
                ...
                if {$secN == 0} {
            	    ...
            	    set sectionMin $Ymin
            	    set sectionMax $floorMin
		    set interval [expr ($Ymin + $floorMin) / 2.0]

		    set volume [expr ($sectionMax - $sectionMin) * $xAve * $zAve]

                } elseif {$secN == [expr $nSection - 1]} {
             
            	    set sectionMin $floorMax
            	    set sectionMax $Ymax
		    set interval [expr ($Ymax + $floorMax) / 2.0]
		    ...
		    set volume [expr ($sectionMax - $sectionMin) * $xAve * $zAve]
            	    ...
            	    set sectionMax [expr $floorMin + $secN]
            	    set sectionMin [expr $sectionMax - 1.0]
