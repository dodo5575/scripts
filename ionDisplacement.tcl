...
proc computeCurrent {frameCurr sel charge dt out} {
...
proc compute {psfPrefix pdbPrefix dcdPrefix xstPrefix outPrefix} {
    ...
    set out {}
    foreach n $nameList {
	lappend out [open "${outPrefix}.ionDisplacement_${n}.dat" w]
    }
    #set outTotal [open "${outPrefix}.ionDisplacement_total.dat" w]
    ...
    for {set f 1} {$f < $nFrames} {incr f} {
        ...
        foreach ion $ionText q $charge o $out {
            ...
            set s [atomselect top "$ion and z < $max and z > $min" frame $f]
    	    computeCurrent $f $s $q $dt $o
    	    #set data [computeCurrent $f $s $q $dt $o]
