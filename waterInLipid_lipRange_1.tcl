...
proc RMSD {psfPrefix dcd xstPrefix outPut} {

    package require pbctools

    variable lipidResname   
    variable dim            
    variable interval       
    variable partitionR     
    variable timestep       
    ...
    for {set i $firstframe} {$i <= $lastframe} {set i [ expr $i + $stride ] } {
        ...
        set sel_t [atomselect top "name OH2 and ($dim > $down and $dim < $up and $a*$a + $b*$b < $outR*$outR) and (within $cutOffFromDNA of nucleic) " frame $i]
        set sel_1 [atomselect top "name OH2 and ($dim > $down and $dim < $up and $a*$a + $b*$b < $outR*$outR) and (within $cutOffFromDNA of nucleic)  and ($a*$a + $b*$b < $partitionR*$partitionR)" frame $i]
        set sel_2 [atomselect top "name OH2 and ($dim > $down and $dim < $up and $a*$a + $b*$b < $outR*$outR) and (within $cutOffFromDNA of nucleic)  and ($a*$a + $b*$b > $partitionR*$partitionR)" frame $i]
...
if {$argc < 12} {
    puts "vmd -dispdev text -e $argv0 -args psfPrefix dcd xstPrefix lipidResname dim interval partitionR timestep lipR outR cutOffFromDNA outPut"
    exit
}
...
set partitionR     [lindex $argv 6]
set timestep       [lindex $argv 7]
set lipR           [lindex $argv 8]
set outR           [lindex $argv 9]
set cutOffFromDNA  [lindex $argv 10]
set outPut         [lindex $argv 11]

RMSD $psfPrefix $dcd $xstPrefix $outPut
