...
def alignBaseNormal(pdb_dict, seq, tName_array, unitDestVec_array, direction):
    ...
    return rotateby(pdb_dict, rotate_matrix, tail_center)
...
def rotation_nt(group, rid, pdb):
    ...
    global data, anchor_atoms_nt, direc
    ...
    pdb_r = rotateby(pdb, rotate_matrix, tail_center) 
    #pdb_r = alignNt(pdb, base_atoms(seq), anchor_atoms_nt, dest_vec, 'x') 
    #pdb_r = alignNt(pdb_r, base_atoms(seq), anchor_atoms_nt, dest_vec, 'y') 
...
#            raise Exception("rotation(group, rid, moved_pdb): Cannot define axis direction:\ngroup = %s\nrid = %s\ncentral = %s" % (group, rid, central))
    ...
    pdb_r = rotateby(pdb_r, rotate_matrix, tail_center) 
...
def rotateby(ori_dict, rotate_matrix, anchor_array):
...
for Seg in key_DnaSegment:
    for k, v in data[Seg].items():
        ...
        if len(data[Seg][k]['ntBond']) == 2:
            ...
            # find 'seq' of the index which direct == 1
            ind = data[Seg][k]['ntBond'][0]
            for Strand in key_DnaStrand:
                if ind in data[Strand]:
                    seq_1 = data[Strand][ind]['seq']
    ...
    for key in data.keys():
        ...
        if key in key_DnaStrand:
            ...
            for k in k_array:
                ...
                ## Translation
                pdb_m = translation_nt(key, k) 
    
                ## Rotation
                pdb_m_r = rotation_nt(key, k, pdb_m) 

                #resid = k_array[::direc][k_array == k] 

                for aname, coor in pdb_m_r.items():
                    index += 1
...
#
#tmp = rotateby(tmp, r_m, anchor)
#print(tmp, np.linalg.norm(dest - tmp['Y']))
#p1 = rotateby({'p1':p1}, r_m, anchor)
