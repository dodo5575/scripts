...
proc computeDistance {sel1 sel2} {
    ...
    set vector [vecsub $v2 $v1]
    set length [veclength $vector]

    return [list $length [lindex $vector 0] [lindex $vector 1] [lindex $vector 2]]
}
...
if {$argc < 8} {
    puts "vmd -dispdev text -e $argv0 -args psfPrefix pdbPrefix dcdPrefix xstPrefix sel1 sel2 timestep outPut"
    exit
}
