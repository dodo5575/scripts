...
function drawSlide(filename,Nx,Ny,Nz,angle,angle2)
...
[x,y,z] = meshgrid(0:Nz-1,0:Ny-1,0:Nx-1);
    xmin = min(x(:)); 
    ymin = min(y(:)); 
    zmin = min(z(:));
...
%hx=contourslice(x,y,z,v,48,[],[],linspace(-0.2,0.3,10));
%set(hx,'EdgeColor',[1.0,1.0,1.0],'LineWidth',.5)
...
for k=1:Nz 
    ...
    z = (k-Nz/2 + 1)*scaleZ - 45;  
           
    if  (( z > 82) || ( z < -25))     % True when above or below the channel
        average = 0.0;
        number = 0;
        for i=1:Nx
            for j=1:Ny
                if ( (i-iCenter)^2 + (j-jCenter)^2 < 12 )
...
%         fprintf(fid,'%g  %g %g\n', z, average/number, Nx/3.0);   
        ...
        while (abs(deltaAverage) < deltaTolerance) & (R < Nx/2-1)
            ...
            if (number ~= numberOld)  % Avoid divide by 0 errors
                ...
                % Gives the percent difference between the new and the old averages
