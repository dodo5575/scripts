...
if {$argc < 8} {
    puts "Usage: ./assignInstancesToGridBinary.tcl skipSteps instanceSteps dt cylinderFile systemGrid outDir outName binFile0 binFile1..."
    puts "skipSteps is the number of steps to skip before recording instances for each file."
    puts "instanceSteps are the lengths of the mini-trajectories that will be output."
    puts "dt is the time between output in the log files."
    ...
    puts "logFile* are binary files containing velocities and forces produced by `tclForcesData.tcl'."
    exit
}
...
set cylFile [lindex $argv 3]
set gridFile [lindex $argv 4]
set outDir [lindex $argv 5]
set outName [lindex $argv 6]
set inFileList [lrange $argv 7 end]
set timestep 1.0
...
proc distributeCorrCyl {gridVar cylVar inFileList outDir outName skipSteps trajSteps dt timestep} {
    upvar $gridVar grid
    upvar $cylVar cyl
    set dt [expr {$timestep*$trajSteps}]
	...
	while {[string length $inBinData] >= $chunk} {
	    ...
	    set line [makeVelocityForce grid $pos0 $pos1 $force [expr {$dt*$readCount}] $dt]
...
set n [distributeCorrCyl grid cyl $inFileList $outDir $outName $skipSteps $instanceSteps $timestep]
