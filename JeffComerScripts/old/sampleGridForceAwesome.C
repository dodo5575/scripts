...
///////////////////////////////////////////////////////////////////////
// Arbitrary boolean geometry class.
class Geometry {
public:
  virtual bool in(Vector3 v) {
    return false;
  }
};

class PoreGeometry : public Geometry {
public:
  PoreGeometry(double length0, double diameter0, double angle0) {
    double pi = 4.0*atan(1.0);
    z0 = 0.5*length0;
    s0 = 0.5*diameter0;
    slope = tan(pi*angle0/180.0);
  }
  
  virtual bool in(Vector3 v) {
    if (fabs(v.z) > z0) return false;
    double rad = s0 + slope*fabs(v.z);
    if (v.x*v.x + v.y*v.y < rad*rad) return false;
...
public:
  ...
  Scatter(const Vector3* point, int n0) {
    n = n0;
    r = new Vector3[n];
    for (int i = 0; i < n; i++) r[i] = point[i];
  }
  
  Scatter(PoreGeometry geo, const Vector3* point, int n0) {
    int i;
    n = 0;
    for (int i = 0; i < n0; i++) {
      if (geo.in(point[i])) n++;
  ...
  const Scatter& operator=(const Scatter& s) {
    delete[] r;
    n = s.n;
    r = new Vector3[n];
    for (int i = 0; i < n; i++) r[i] = s.r[i];
...
public:
  ...
  virtual Scatter nodes(const PoreGeometry& geo) const {
    Vector3* r = new Vector3[size];
    
    int j = 0;
    for (int ix = 0; ix < nx; ix++) {
      for (int iy = 0; iy < ny; iy++) {
	for (int iz = 0; iz < nz; iz++) {
	  r[j] = basis.transform(Vector3(ix, iy, iz)) + origin;
	  j++;
	}
      }
    }
    
    Scatter ret(geo, r, size);
    delete[] r;
    return ret;
  }
...
int main(int argc, char* argv[])
{
  ...
  if ( argc != 7 ) {
    printf(
      "Usage: %s basisFile dx poreLength poreDiameter poreAngle outFile\n",
      argv[0]);
    return 0;
  }
  int outFileIndex = 6;
  int basisFileIndex = 1;
  ...
  Scatter sysVec(argv[basisFileIndex]);
  ...
  Grid sourceGrid(sys, origin, 1.0);
  PoreGeometry geo(poreLength, poreDiameter, poreAngle);
  if (geo.in(Vector3(1.0))) printf("Center is in!!!");
  // Get the source points.
  Scatter source = sourceGrid.nodes(geo);
  printf("\nLoaded %i source points.\n", source.length());
  source.write(argv[outFileIndex]);
