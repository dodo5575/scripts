...
public:
  ...
  void add(const IndexList& l) {
    ...
    if (num > maxnum) {
      ...
      for(int i = 0; i < oldnum; i++) list[i] = oldlist[i];
      delete[] oldlist;
    }

    for(int i = 0; i < l.num; i++) list[i+oldnum] = l.list[i];
  }
...
public:
  ...
  IndexList getCellContents(int j) {
    return cell[j].point;
  }
...
int main(int argc, char* argv[])
{
  ...
  double cutoff = 2.0*(radius+sigma);
