...
public:
  ...
  static int countCoordinates(const char* fileName) {
   ...
   // Open the file.
    ...
    if (inp == NULL) {
      printf("Error! Scatter:countCoordinates Couldn't open file %s\n.",fileName);
      exit(-1);
    }
...
private:
  ...
  virtual void readCoordinates(const char* fileName, int num, Vector3* r) {
    ...
    FILE* inp = fopen(fileName,"r");
    if (inp == NULL) {
      printf("Error! Scatter:readCoordinates Couldn't open file %s\n.",fileName);
      exit(-1);
    }
  ...
  virtual void readTrajectory(const char* fileName, int num, Vector3* r, double cutTime) {
    ...
    FILE* inp = fopen(fileName,"r");
    if (inp == NULL) {
      printf("Error! Scatter:readTrajectory Couldn't open file %s\n.",fileName);
      exit(-1);
    }
...
public:
  ...
  virtual void profileZ(double factorX, double factorY, const char* fileName) {
    ...
    FILE* out = fopen(fileName,"w");
    if (out == NULL) {
      printf("Error! Grid::profileZ Couldn't open file %s\n.",fileName);
      return;
    }
  ...
  virtual void averageProfileZ(const char* fileName) {
    ...
    FILE* out = fopen(fileName,"w");
    if (out == NULL) {
      printf("Error! Grid:averageProfileZ Couldn't open file %s\n.",fileName);
      exit(-1);
    }
  ...
  virtual void depthMapZ(const char* fileName) {
    ...
    // The first line of the profile file contains the z-values. 
    fprintf(outProfile, "%.10g", origin.z);
    for (int iz = 1; iz < nz; iz++) {
      Vector3 r = basis.transform(Vector3(0,0,iz)) + origin;
      fprintf(outProfile, " %.10g", r.z);
    }
    fprintf(outProfile, "\n");
