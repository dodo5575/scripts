...
int mainPaste(int argc, char* argv[]) {
  if ( argc != 10 ) {
    printf("Usage: %s srcFile destFile x0 y0 z0 x1 y1 z1 outputFile\n", argv[0]);
    return 0;
  }
  ...
  r0.x = strtod(argv[3],NULL);
  r0.y = strtod(argv[4],NULL);
  r0.z = strtod(argv[5],NULL);
  r1.x = strtod(argv[6],NULL);
  r1.y = strtod(argv[7],NULL);
  r1.z = strtod(argv[8],NULL);
  
  printf("Keeping from (%s) to (%s).\n", r0.toString().val(), r1.toString().val());
  src.crop(r0,r1);
  dest.paste(src);
  dest.write(argv[argc-1], "pasted grid");
}


int main(int argc, char* argv[]) {
  return mainPaste(argc, argv);
}
