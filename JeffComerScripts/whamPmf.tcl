...
proc wham {xVar binN winCen winSpring outFile levelX0 levelX1 {tol 0.001}} {
    upvar $xVar x
    set energyThreshold 0.8
	...
	if {$energy < $energyThreshold} {
	    ...
	    puts "Ignoring data! Average energy is $energy kT."
	}

	# Decide whether to keep the window or not.
	if {$histN > 0 && $energy < $energyThreshold} {
	    lappend winList $w
		...
		lappend freq [expr double($count($i))/$histN/$binSize]
	    }
	...
	foreach w $winList {
		...
		set uBias [expr 0.5*$ws*$dr*$dr]
	        set sum [expr $sum + $binSize*exp(-$uBias)*[lindex $prob $b]]
	    }
	...
	if {$z >= $levelX0 && $z < $levelX1} {
	    set levelMean [expr $levelMean + $u]
	    incr levelCount
	}
    }
    set levelMean [expr $levelMean/$levelCount]
    puts "Zero level: $levelMean"

    set out [open $outFile w]
    foreach pair $pmf {
	puts $out "[lindex $pair 0] [expr [lindex $pair 1] - $levelMean]"
    }
    close $out
