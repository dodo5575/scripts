...
if {$argc < 5} {
    puts "$argv0 name structPrefix outputDir dcdFreq stride windowRadius dcdFile0 [dcdFile1...]"
    exit
}
...
proc compute {name moiety structPrefix dcdList dcdFreq outDir stride windowRad} {
    ...
    set cellVec [readXsc $xsc]
    foreach {lx ly lz}  $cellVec { break }
    puts "lx: $lx"
    puts "ly: $ly"
    puts "lz: $lz"
...
compute $name $moiety $structPrefix $dcdList $dcdFreq $outDir $stride $windowRad
