...
# Get the unique run names.
set fileList [glob ${prefix}*.dat]
set nameList {}
foreach f $fileList {
    set name [trimExtension [trimExtension $f]]
    lappend nameList $name
}
set nameList [lsort -unique $nameList]
#set nameList [lrange $nameList 0 0]

# Average the files.
foreach name $nameList {
    puts $name
    set thisList [glob ${name}*.dat]

    # Read the files.
    set trajList {}
    foreach this $thisList {
	lappend trajList [readData $this]
    }
    ...
    set out [open ${outPrefix}_${name}.dat w]
