...
proc getBaseBasis0 {segName resId {mole top}} {
    set selText "segname $segName and resid $resId"
...
proc getBaseBasis {segName resId {mole top}} {
	...
	set selY0 [atomselect $mole "($selText) and name N1"]
...
# Map coordinates from one molecule to another.
proc mapCoordinatesTemp {tempMol targMol seg res nameList {selText "all"}} {
    # Handle nameList == all.
    if {[string equal $nameList all]} {
	set s [atomselect $targMol "segname $seg and resid $res and ($selText)"]
	set nameList [$s get name]
	$s delete
    }
    set missList {}
...
proc fixMutantDna {tempPdb tempText targPdb targText outPdb} {
	...
	if {[string equal $targResName $tempResName]} {
	    ...
	    mapCoordinatesTemp $loadMol($targResName) $targMol $targSeg $targRes all "not name $backboneAtoms"
	    ...
	    set s [atomselect $targMol "segname $targSeg and resid $targRes and not name $backboneAtoms"]
