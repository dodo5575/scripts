...
switch $moiety {
	...
	set nBasepairs 2; # last basepair is resA0+nBasepairs-1 and resB0-nBasepairs+1
    }
    dsDNA {
	set segA ADNA
	set segB BDNA
...
set degreesPerRadian [expr 45.0/atan(1.0)]
...
set out [open output/last_hbond_${outSuffix} w]
set outAngle [open output/last_base_angle_${outSuffix} w]
...
foreach a $resA b $resB rnA $resNameA rnB $resNameB {
	...
	if {[string equal $baseB $rnB]} {
		...
		lappend hbA [atomselect top "index $iA"]
		lappend hbB [atomselect top "index $iB"]
		incr nBonds
...
foreach dcdFile $dcd {
	...
	set angle 0.0
	foreach a $hbA b $hbB {
	    set ra [lindex [$a get {x y z}] 0]
	    set rb [lindex [$b get {x y z}] 0]
	    set d [vecsub $rb $ra]
	    set dMag [veclength $d]

	    if {$dMag < $cutoff} {
		incr n
		set angle [expr $angle + $degreesPerRadian*acos([lindex $d 2]/$dMag)]
	    }
	}
	# Compute the average angle.
	if {$n > 0} {set angle [expr $angle/$n]}

	# Write the time and distance.
	puts $out "$t $n"
	puts $outAngle "$t $angle"
