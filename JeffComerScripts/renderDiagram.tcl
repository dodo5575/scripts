...
proc drawWindows {windowFile scale {mole top}} {
    set kT 0.5862292; # in kcal/mol at 295 K
    set winData [readData $windowFile]

    foreach item $winData {
	foreach {ind rx ry rz kx ky kz} $item { break }
	set spring [expr {($kx+$ky+$kz)/3.0}]
	set rad [expr {sqrt($scale*$kT/$spring)}]
	
	graphics $mole sphere [list $rx $ry $rz] radius $rad resolution 20
    }
}
