#!/usr/bin/env perl
die "Usage: $0 configFile jobs map0 map1\n" unless @ARGV == 4;

my $execFile = "/home/jcomer/bin/browntown";
my $pmfFiles = "@ARGV[2], @ARGV[3], files/long_chl-chl.dat, files/long_pot-chl.dat, files/long_pot-pot.dat, files/coords_no_160.txt";
my $prefix = @ARGV[0];
my $jobs = @ARGV[1];
