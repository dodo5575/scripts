...
proc makeSegName3 {segName0 index} {
    set base36 "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    set seg [string index $segName0 0]

    set high [expr {$index/(36*36)%36}]
    set mid [expr {($index/36)%36}]
    set low [expr {$index%36}]
    
    set s0 [string index $base36 $high]
    set s1 [string index $base36 $mid]
    set s2 [string index $base36 $low]
...
proc main {} {
	...
	foreach psfLine $psfAtoms pdbLine $pdbAtoms r $pos {
	    ...
	    set segName [makeSegName3 $oldSegName $j]
	    set r1 [vecAdd $r $r0]
