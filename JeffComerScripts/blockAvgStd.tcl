...
if {$argc < 2} {
    puts "Usage: tclsh blockAvgStd.tcl windowSize inputFile0 \[inputFile1\]..."
    exit
}
...
foreach inFile $inFileList {
	...
	if {[string match "#*" $line]} {
		...
		set errY [expr sqrt(($sumYY - $sumY*$sumY/$n)/($n-1))] 
    ...
    close $out
    close $in
    puts "The block averages and standard deviations were successfully written to `${outFile}'."
}
