...
foreach pos $instPosList {
    ...
    set inRegion [expr {$s2 < $rad*$rad && abs($z) < $z0}]
    if {$notLogic} {
	set inRegion [expr !$inRegion]
    } 
...
if {$doVelocity} {
    ...
    puts "Wrote average velocity correlation function for the entire system to $outFile."

    # Integrate the correlation function to obtain the diffusivity.
    set sum 0.0
    foreach corr $meanCorrVelAll {
	set sum [expr $sum + $corr*$dt]
    }
    # Scale by the number of dimensions.
    set diffuse [expr {$sum/3.0}]
    # Convert to A^2/ns. Was in A^2/fs.
    set diffuse [expr {$diffuse*1e6}]
    puts "diffusivity_vel $diffuse A^2/ns."
...
if {$doForce} {
	...
	set meanForceTraj [meanTrajListVec $forceTrajList]

	# Compute the deviation of the force F(r,t) - <F(r,t)>.
	set deltaForceTrajList [addTrajListVec $forceTrajList $meanForceTraj -1.0]
	...
	if {$sum == 0.0} {
	    # This usually happens when there is only one trajectory.
	    continue 
	}
    ...
    # Integrate the correlation functions to obtain the diffusivity.
    set sum 0.0
    foreach corr $meanCorrForceAll {
	set sum [expr $sum + $corr*$dt]
    }
    set diffuse [expr {$kT*$kT/$sum}]
    # Convert to A^2/ns. Was in A^2/fs.
    set diffuse [expr {$diffuse*1e6}]
    puts "diffusivity_force $diffuse A^2/ns."
