...
int main(int argc, char* argv[]) {
  if ( argc != 5 ) {
    printf("Usage: %s srcGrid scaleFactor voltageDrop x-component factor y factor z factor outFile\n", argv[0]);
    return 0;
  }
  ...
  double xfactor = strtod(argv[4],NULL);
  double yfactor = strtod(argv[4],NULL); 
  double zfactor = strtod(argv[5],NULL); 
  ...
  double lx =  src.getExtent().x;              
  double ly = 113;             //obtaining y coordinate does not work for graphene-defect system.
  double lz = src.getExtent().z;   
  printf("The length of the system is %g.\n", lx);
  src.addGradient(Vector3(-voltage/lx*xfactor , voltage/ly*yfactor, voltage/lz*zfactor ));
